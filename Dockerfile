...
FROM --platform=${TARGETPLATFORM:-linux/arm64} alpine:3.10
...
RUN printf "I am running on ${BUILDPLATFORM:-linux/arm64}, building for ${TARGETPLATFORM:-linux/arm64}\n$(uname -a)\n"
...
RUN apk --update --no-cache add \
    ...
    tar \
    tzdata \
  && S6_ARCH=$(case ${TARGETPLATFORM:-linux/arm64} in \
...
RUN mkdir -p /opt/flarum \
  ...
  && cd /opt/flarum && COMPOSER_CACHE_DIR="/tmp" composer create-project flarum/flarum /opt/flarum --stability=beta \
